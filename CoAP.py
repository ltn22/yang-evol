...
# To allow rule selection, especially with MSB the size must be controlled
    ...
    def __add_option_TL ( self, T, L ):
        ...
        if ( delta < 13 ) and ( L < 13 ) is True:
            ...
            if c_delta == 14:
                self.buffer += struct.pack ('!H', delta)
    ...
    def dump(self, hexa=False):
        ...
        while buf_ptr < len(self.buffer):
            ...
            if delta_option == 13:
                ...
                buf_ptr += 1
            if delta_option == 14:
                b1=self.buffer[buf_ptr]
                b2=self.buffer[buf_ptr+1]
                delta_option = ((b1<<8) | b2) + 269
                buf_ptr += 2

            if length_option == 13:
                length_option = self.buffer[buf_ptr]+13
                buf_ptr += 1
